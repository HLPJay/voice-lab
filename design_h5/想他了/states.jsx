...
function StateSheet({ tone = 'accent', title, body, primary, secondary, tertiary, illustration }) {
  const tones = {
  ...
  return (
    <div className="xt-screen" style={{ paddingTop: 64, paddingBottom: 30 }}>
      ...
      <div style={{ padding: '36px 16px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {primary  && <button className="xt-btn primary full" style={{ height: 50 }}>{primary}</button>}
        {secondary && <button className="xt-btn ghost full">{secondary}</button>}
...
const Spin = (
  <div style={{
    ...
    background: `conic-gradient(${T.accent} 70%, transparent 30%)`,
