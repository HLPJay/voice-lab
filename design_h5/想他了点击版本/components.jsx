...
// ─────────────────────────────────────────────────────────────
function MiniPlayer({ title, recipient, scene, letterId, totalSecs: totalSecsProp, total: totalProp }) {
  // Parse "M:SS" → seconds; format back.
  const toSec = (s) => { const [m, ss] = String(s).split(':').map(Number); return (m * 60) + (ss || 0); };
  const toMS  = (n) => { const m = Math.floor(n / 60), s = Math.max(0, Math.round(n) % 60); return `${m}:${String(s).padStart(2, '0')}`; };
  const totalSecs = Math.max(1, totalSecsProp ?? toSec(totalProp || '1:08'));

  // Use the global playback singleton when a letterId is provided
  // (history docked / home recent / letter detail all share the same scrubber).
  // Without a letterId we fall back to local state — used in screenshots.
  const usingGlobal = !!letterId;
  const pb = usePlayback(letterId || null, totalSecs);
  const isCurrent = usingGlobal && pb.letterId === letterId;
  const playing = isCurrent ? pb.playing : false;
  const secs    = isCurrent ? pb.secs : 0;
  const progress = totalSecs > 0 ? Math.min(1, secs / totalSecs) : 0;

  const toggle = (e) => {
    e.stopPropagation();
    if (!usingGlobal) return;
    Playback.toggle();
  };
  ...
  return (
    <div style={{
      ...
      <div className="xt-row xt-gap-3">
        <button onClick={toggle} aria-label={playing ? '暂停' : '播放'} style={{
          width: 40, height: 40, borderRadius: 999, border: 0, padding: 0,
          background: 'var(--xt-accent)', cursor: 'pointer',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: '0 4px 14px var(--xt-accent-soft)',
          flexShrink: 0,
        }}>
          {playing
            ? <svg width="14" height="14" viewBox="0 0 14 14"><title>暂停</title><rect x="3" y="2" width="3" height="10" rx="1" fill="white"/><rect x="8" y="2" width="3" height="10" rx="1" fill="white"/></svg>
            : <svg width="14" height="14" viewBox="0 0 14 14"><title>播放</title><path d="M3 2l9 5-9 5V2z" fill="white"/></svg>}
        </button>
        <div className="xt-grow" style={{ minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: T.text3, marginTop: 1, letterSpacing: 0.04 }}>
            {recipient} · {scene} · {toMS(secs)} / {toMS(totalSecs)}
          </div>
        </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: 'var(--xt-accent)', borderRadius: 999,
...
// ─────────────────────────────────────────────────────────────
function FullPlayer({ totalSecs = 24, voice = '温柔女声', tone = '真诚', letterId, onDownload }) {
  ...
  const bars = React.useMemo(() => {
    ...
    return Array.from({length: N}, (_, i) => {
      ...
      const syl = 0.55 + 0.45 * Math.sin(i * 0.42 + 1.3);
      const fine = 0.5 + 0.5 * Math.sin(i * 1.18);
      const gap = (i === 21 || i === 22 || i === 44) ? 0.18 : 1;
      const v = env * (syl * 0.65 + fine * 0.35) * gap;
      return Math.max(0.06, Math.min(1, v));
    });
  }, []);

  const pb = usePlayback(letterId, totalSecs);
  const isCurrent = pb.letterId === letterId;
  const playing = isCurrent ? pb.playing : false;
  const secs    = isCurrent ? pb.secs : 0;
  const progress = totalSecs > 0 ? Math.min(1, secs / totalSecs) : 0;
  const playedBars = Math.floor(bars.length * progress);

  const toMS = (n) => `${Math.floor(n/60)}:${String(Math.max(0, Math.round(n)%60)).padStart(2,'0')}`;
  const toggle = () => Playback.toggle();
  const restart = () => { Playback.seek(0); Playback.play(); };

  // Scrub: click + pointer drag along waveform
  const wfRef = React.useRef(null);
  const seekFromEvent = (e) => {
    if (!wfRef.current) return;
    const r = wfRef.current.getBoundingClientRect();
    const clientX = e.touches ? e.touches[0].clientX : e.clientX;
    Playback.seek((clientX - r.left) / r.width);
  };
  ...
  return (
    <div className="xt-card-elev" style={{ padding: 18, margin: '0 16px' }}>
      <div className="xt-row" style={{ justifyContent: 'space-between', marginBottom: 14 }}>
        ...
        <span style={{ fontSize: 11, color: T.text3, fontFamily: T.mono }}>{toMS(secs)} / {toMS(totalSecs)}</span>
      </div>
      <div
        ref={wfRef}
        onPointerDown={onPointerDown}
        style={{ position: 'relative', height: 52, marginBottom: 14, cursor: 'pointer', touchAction: 'none' }}
      >
        <div style={{
          ...
          {bars.map((v, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                background: played
                  ? (isHead ? T.accentInk : 'var(--xt-accent)')
                  : T.hairline2,
        ...
        <div style={{
          ...
          boxShadow: `0 0 8px var(--xt-accent)`,
          pointerEvents: 'none',
        }} />
      </div>
      <div className="xt-row" style={{ justifyContent: 'space-between', gap: 16 }}>
        <button onClick={restart} className="xt-btn ghost" style={{ flex: 1, padding: '10px 12px' }}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><title>从头</title>
            <path d="M2 8l4-4v3h8v2H6v3l-4-4z" fill="currentColor"/>
          </svg>
          从头听
        </button>
        <button onClick={toggle} aria-label={playing ? '暂停' : '播放'} style={{
          width: 56, height: 56, borderRadius: 999, border: 0, padding: 0,
          background: 'var(--xt-accent)', cursor: 'pointer',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: '0 10px 28px var(--xt-accent-soft), inset 0 1px 0 rgba(255,255,255,0.16)',
        }}>
          {playing
            ? <svg width="20" height="20" viewBox="0 0 20 20"><title>暂停</title><rect x="5" y="3" width="3.5" height="14" rx="1" fill="white"/><rect x="11.5" y="3" width="3.5" height="14" rx="1" fill="white"/></svg>
            : <svg width="20" height="20" viewBox="0 0 20 20"><title>播放</title><path d="M5 3l11 7-11 7V3z" fill="white"/></svg>}
        </button>
        <button onClick={onDownload} className="xt-btn ghost" style={{ flex: 1, padding: '10px 12px' }}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><title>下载</title>
...
// ─────────────────────────────────────────────────────────────
function ExpressionCard({ style, fitsFor, text, active, length = '32字', onSelect }) {
  return (
    <div onClick={onSelect} style={{
      ...
      <div className="xt-row xt-gap-2" style={{ flexWrap: 'wrap' }}>
        <button onClick={(e) => e.stopPropagation()} className="xt-btn ghost" style={{ padding: '8px 12px', fontSize: 12, borderRadius: 999 }}>
          编辑
        </button>
        <button onClick={(e) => e.stopPropagation()} className="xt-btn ghost" style={{ padding: '8px 12px', fontSize: 12, borderRadius: 999 }}>
          复制
        ...
        <button onClick={(e) => { e.stopPropagation(); onSelect && onSelect(); }} className="xt-btn" style={{
...
// ─────────────────────────────────────────────────────────────
function AppBar({ title, leftAction = 'back', rightAction = 'more', subtitle, transparent = false, onLeft, onRight }) {
  return (
    <div style={{
...
function StepDots({ active = 0, total = 4, labels = [] }) {
  // Connected progress strip: one shared track, accent dot per stage, line under.
  const pct = total > 1 ? (active / (total - 1)) * 100 : 0;
  return (
    <div style={{ padding: '6px 22px 0', marginTop: 4 }}>
      {/* track */}
      <div style={{ position: 'relative', height: 12 }}>
        <div style={{
          position: 'absolute', left: 0, right: 0, top: '50%',
          height: 1, background: T.hairline2, transform: 'translateY(-50%)',
        }} />
        <div style={{
          position: 'absolute', left: 0, top: '50%', width: `${pct}%`,
          height: 1.5, background: T.accent, transform: 'translateY(-50%)',
        ...
        {Array.from({length: total}).map((_, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              width: cur ? 12 : 6, height: cur ? 12 : 6, borderRadius: 999,
              background: done || cur ? T.accent : T.surface2,
              border: cur ? `2px solid ${T.bg}` : `1px solid ${done ? T.accent : T.hairline2}`,
              boxShadow: cur ? `0 0 0 3px ${T.accentSoft}` : 'none',
              transition: 'all .25s',
            }} />
          );
        })}
      ...
      <div className="xt-row" style={{ justifyContent: 'space-between', marginTop: 8 }}>
        {labels.map((l, i) => (
          <span key={i} style={{
            ...
            color: i <= active ? T.text2 : T.text3,
