...
* P16-PROVIDER-MODEL-BINDING-B1-CHECK：验证最小 model/binding 可见性与恢复增强已完成 ✅
...
* P16-XIAOMI-MIMO-TTS-REAL-PROBE-B2：真实 API 探测执行（用户手动执行成功）已完成 ✅
* P16-XIAOMI-MIMO-TTS-ADAPTER-TEST-TRIAGE-C0：adapter 测试失败排查（commit d8df563，153 passed）已完成 ✅
* 当前下一阶段：P16-XIAOMI-MIMO-TTS-PROBE-DUPLICATE-CALL-TRIAGE-D1（可选排查）
* 当前不进入：B3（adapter render_sync 验证）需 D1 完成后评估
...
* REAL-PROBE-B2 当前风险：probe real-call 日志重复打印（需 D1 排查）
...
P15 统计模块定位为创作行为统计 + 本地资产观察 + 使用情况分析，用于回答"我生成了多少？哪些模式用得最多？context 积累了多少？"

### 已审查数据源

**SampleStore**（`voice_lab_recent_samples_v1`，最多 200 条）：
- 字段：sample_id / created_at / source / provider / profile_name / audio_format / duration_ms / status / context_id / asset_id / download_url
- source 枚举：workspace_sync/async/stream/variant, audition, batch_longtext/script_merged/segment, unknown

**ContextStore**（`voice_lab_sample_context_v1`，最多 50 条）：
- longtext 字段：full_text / provider / profile_id / segment_strategy / max_segment_chars / silence / audio_format / need_subtitle / params
- script 字段：lines / provider / silence / audio_format / need_subtitle

**现有 Tab**（无统计 Tab）：workspace / longtext / script / voices / history / advanced

**后端 API**（只读，不修改）：voice_assets / voice_jobs / voice_cost / runtime_status / admin — 本阶段只记录存在，不依赖

### A/B/C 指标分级

**A 级（基于 SampleStore 直接实现）**：
最近样本总数 / 今日生成 / 近 7 天 / 近 30 天 / source 分布 / provider 分布 / profile_name 分布 / audio_format 分布 / 总时长 / 平均时长 / 有 context_id 数 / 有 asset_id 数 / 有 download_url 数

**B 级（基于 ContextStore 辅助实现）**：
context 总数 / longtext 数量 / script 数量 / longtext 总字数 / 平均字数 / script 总行数 / 平均行数 / 角色数估算 / subtitle 开启比例 / audio_format 分布 / segment_strategy 分布 / context 覆盖率

**C 级（后续后端支持）**：
全量资产数量/体积/时长 / 完整日期趋势 / 失败任务数 / 失败原因排行榜 / Provider 成功率 / 成本估算 / token 消耗 / 多用户统计

### 推荐展示位置

独立 Tab（`统计`），不放入 SampleSidebar。SampleSidebar 继续作为操作入口。

### 数据边界

SampleStore 上限 200 条，ContextStore 上限 50 条。统计只反映本地最近使用情况。UI 必须标注"基于本地最近 N 条样本，不代表全量历史"。

### 非阻塞观察项

- A0-OBS-001：context 覆盖率可分为"样本带 context_id"（分子）与"ContextStore 可实际找到 context"（需二次查询）两个指标，当前设计使用前者，合理。
- A0-OBS-002：source 枚举未来可能扩展，StatsStore 应把未知 source 归入 unknown。
- A0-OBS-003：duration_ms 可能为 null，总时长与平均时长仅代表可统计样本，应在 UI 上注明。
- A0-OBS-004：profile_name 可能缺失，展示时应 fallback 到 profile_id 或 unknown。
- A0-OBS-005：script role 可能为空，应统计非空 role，空 role 归入 unknown。

### 不进入范围

不实现 UI / stats_store.js / 后端接口 / 数据库表 / SaaS 统计 / 成本估算 / 图表库 / 数据导出 / 资产扫描 / 失败归因。
...
SampleStore / ContextStore 数据源描述准确。A/B/C 级指标分级合理。展示位置为独立 Tab。第一版不引入图表库或后端依赖。数据边界已补充完整边界说明（覆盖 SampleStore 200 + ContextStore 50 上限）。P15-B1 实现建议小而可控。无阻塞设计问题。

### 复核通过确认

- SampleStore key/limit/fields 全部匹配 ✓
- ContextStore key/limit/fields 全部匹配 ✓
- A 级 13 个指标全部可直接计算 ✓
- B 级 12 个指标全部可计算（需注意 OBS 项）✓
- C 级 11 个指标正确后置 ✓
- 独立 Tab 结论合理 ✓
- 第一版不做图表/后端/导出 ✓
- 数据边界已明确（已补充完整说明）✓
- P15-B1 stats_store.js 职责边界清晰 ✓
- P15-B1 stats.js 职责边界清晰 ✓
- 非阻塞观察项已记录（5 条）✓

### 阶段状态

P15-STATS-A0-CHECK 通过。当前阶段推进到 P15-STATS-B1-A0。

## P15-STATS-B1-A0：轻量本地统计面板实现前置审查

### 阶段定位

在实现 stats_store.js 和 stats.js 之前，先审查实现范围，确保实现受控。

### 前置审查项

- stats_store.js 纯计算模块边界
- stats.js tab panel DOM 结构
- index.html 新增 tab button 和 container
- switchTab 对 data-tab="stats" 的支持
- 测试覆盖策略
...
2. P15-B1 本地统计（SampleStore 最多 200 条，ContextStore 最多 50 条）范围有限，不足以反映全量历史。
3. 当前产品阶段更重要是保持 P14 创作闭环稳定。
4. 统计能力应根据后期产品使用情况再决定是否启动。

### 管理面板已有统计能力

后端接口：/api/admin/stats/summary、/api/admin/stats/daily、/api/admin/call-logs

已有指标：总任务数 / 成功数 / 失败数 / 成功率 / 总字符数 / 总音频时长 / Provider 统计 / API 分布 / 每日趋势 / 最近调用记录 / 最近错误

### P15 本地统计限制

SampleStore 上限 200 条，ContextStore 上限 50 条。只代表当前浏览器最近使用，不代表后端全量历史、真实资产总量、真实成本或真实失败率。

### 后期可选方向

- 方向 A（轻量）：本地最近创作统计 — 前端实现成本低，统计范围限于本地
- 方向 B（重投入）：服务端全量统计 — 需后端数据库和任务记录体系支持

### 文档保留

已完成的设计文档（P15_STATS_A0_DESIGN.md、P15_STATS_B1_A0_IMPLEMENTATION_AUDIT.md）保留备查，不删除。

### 阶段状态

P15-STATS-B1-PARK 完成。当前阶段推进到 NEXT-PRIORITY-REVIEW。
...
**流式取消卡死**：`handleGenerate()` 在 `startStreamGenerate()` 之前执行 `setLoading(true)`，`startStreamGenerate()` 的 cancel 直接 return，无 `setLoading(false)` 路径。

**t2a 无确认**：`t2a` 不在 `_OPERATION_MESSAGES` 中，普通单条无费用确认。

**quick preview 绕过**：`voice_clone.js` quick preview 直接 `fetch('/api/voice/render')`，不走 `guardedJsonFetch`，无确认。

**确认前副作用**：所有 `guardedJsonFetch` 入口在 confirm 前执行 setLoading(true) / 清空 resultsArea / stopAsyncPolling()。

### P1 根因

**多版本无等待态**：variants 是同步请求，`renderResults` 在接口返回后才渲染，resultsArea 全程空白。

**样本只恢复文本**：`fillTextInput()` 只写 `#textInput`，`buildWorkspaceSampleContext` 未保存 speed/vol/pitch/emotion/need_subtitle/genMode/outputFormat/variantCount。

### 后续阶段建议

P16-CANCEL-A0-CHECK → P16-CANCEL-FIX1（取消语义和 loading 卡死修复）→ P16-VARIANTS-UX-FIX1（多版本等待态）→ P16-WORKSPACE-RESTORE-A0/B1（样本参数恢复）

### 阶段状态

P16-REAL-USAGE-ISSUES-A0 完成。当前阶段推进到 P16-CANCEL-A0-CHECK。

## P16-CANCEL-A0-CHECK：取消确认与生成状态问题复核

### 复核结论

**通过**。所有 A0 问题描述均与实际代码一致。

### 复核确认

- **流式取消卡死**：确认。`startStreamGenerate()` cancel 直接 return，无 `setLoading(false)`。index.html line 3300-3301。
- **确认前副作用**：确认。`handleGenerate()` lines 3220-3224 在 `guardedJsonFetch` 之前执行 `setLoading(true) / stopAsyncPolling() / resultsArea.innerHTML = ''`。
- **旧轮询停止**：确认。`stopAsyncPolling()` 在 confirm 前执行，sets `asyncPollingState.stopped = true`。index.html lines 3221/3168-3171。
- **t2a 无确认**：确认。`_OPERATION_MESSAGES` 不含 `t2a`，普通单条 minimax TTS 无确认。index.html lines 2832-2843/3258。
- **quick preview 绕过**：确认。voice_clone.js line 321 / voice_design.js line 179 / voice_import.js line 184 直接 `fetch`，无确认。
- **多版本无等待态**：确认。variants 同步 HTTP 请求，接口返回前 resultsArea 空白。index.html lines 3257/3278。
- **workspace 参数恢复**：确认。`fillTextInput()` 只写 textInput，无参数恢复。

### P16-CANCEL-FIX1 边界

纳入：流式取消 setLoading(false) / t2a 确认 / quick preview guardedJsonFetch / 确认前副作用统一
不纳入：workspace 参数恢复 / 剧本扩展 / 统计模块
可选：多版本等待态（可并入或拆出）
...
复核 P16-PROVIDER-BOUNDARY-A0 审查文档（提交 0de1f3a）是否完全符合代码事实。

### 复核结论：通过，发现 1 处表述需修正

**需修正**：第 11 节横向审查表中，`voice_render_service` 行 CostGuard 列应修正为：

原表述：`✅ resolved_provider`（CostGuard）
修正为：`⚠️ estimate_t2a_cost (非 require_confirmed)` — `VoiceRenderService.render_voice()` 不调用 `require_confirmed()`，只调用 `estimate_t2a_cost()` 做成本估算。普通 t2a sync 不在 HIGH_RISK_OPERATIONS，后端无强制校验。

### 各风险复核结果

| 风险 ID | 结论 |
|---|---|
| P16-PROVIDER-RISK-001（Mock fallback） | ✅ 表述准确 |
| P16-PROVIDER-RISK-002（VoiceVariantService CostGuard 绕过） | ✅ 表述准确 |
| P16-PROVIDER-RISK-003（t2a sync 无后端强校验） | ✅ 表述准确 |
| 前端 Provider 下拉硬编码 | ✅ 表述准确 |
| Capability 模型 | ✅ 表述准确 |
...
将 Provider 接入从硬编码改造为配置驱动，为后续 OpenAI、Azure、火山引擎、阿里云等语音大模型接入做准备。

### 核心改造

**改造前（硬编码）**：
```
PROVIDER_REGISTRY = {"mock": MockAdapter, "minimax": MiniMaxAdapter}
COST_PROVIDER_SET = frozenset({"minimax"})
```

**改造后（配置驱动）**：
```
provider name -> ProviderConfig (YAML) -> adapter_type -> ADAPTER_TYPE_REGISTRY -> Adapter class
ProviderConfig.real_cost -> CostGuardService (替代 COST_PROVIDER_SET)
```

### 关键设计

1. **ProviderConfig schema** (`app/domain/provider_config.py`)
   - `name`, `display_name`, `enabled`, `adapter_type`, `real_cost`
   - `api_key_env`, `base_url_env`, `base_url`, `endpoints`
   - `default_model`, capability toggles, `metadata`

2. **配置文件**：`config/providers.yaml`
   - 初始包含 `mock`, `minimax`, `mock_configured`
   - 不暴露 `api_key` 真实值，只暴露 env 变量名

3. **provider_config_loader** (`app/config/provider_config_loader.py`)
   - `list_provider_configs()`, `get_provider_config(name)`
   - `list_enabled_provider_configs()`, `clear_provider_config_cache()`

4. **Adapter Type Registry** (`app/providers/adapter_type_registry.py`)
   - `adapter_type -> Adapter class`，解耦 provider name 和 adapter class
   - `PROVIDER_REGISTRY` 不再用于运行时路由（保留向后兼容）

5. **Capability Registry**：`ProviderConfig + adapter_type builder -> ProviderCapability`
   - `/api/voice/capabilities` 响应格式不变
   - metadata 不允许包含 secret/token/password

6. **Cost Guard**：用 `ProviderConfig.real_cost` 替代 `COST_PROVIDER_SET`

7. **前端影响**：high-risk 确认从 `provider === "minimax"` 硬编码改为通过 capability metadata 判断（属于 B1 范围）

### B1 实现建议

实现步骤：
1. 新增 `app/domain/provider_config.py`, `app/config/provider_config_loader.py`, `app/providers/adapter_type_registry.py`
2. 新增 `config/providers.yaml`（包含 mock, minimax, mock_configured）
3. 改造 `get_provider()` 由配置驱动，保留 fallback 向后兼容
4. 改造 `_build_registry()` 从 YAML 读取
5. 改造 `CostGuardService` 用 `real_cost`
6. 添加 `mock_configured` 端到端验证

B1 验证：`get_provider("mock_configured")` 返回可用 `MockSpeechAdapter`，`/api/voice/capabilities` 包含 `mock_configured`。

### 未纳入范围

- OpenAI / Azure / 火山 / 阿里云实际接入
- 管理 UI（增删改 provider 配置）
- 运行时配置热更新
- Pricing metadata
- 前端 high-risk 确认改造（属于 B1 前端部分）
...
| 文件 | 用途 |
|---|---|
| `config/providers.yaml` | Provider 配置文件 |
...
**修复：disabled provider 未检查 enabled 状态**

`get_provider(name)` 在配置中找到 provider 时，未检查 `ProviderConfig.enabled` 是否为 `false`。当 `enabled=false` 时，仍返回 adapter 实例。
