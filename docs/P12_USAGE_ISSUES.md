...
## P12-USAGE-UX5：分段策略文案误导

**发现时间：** 2026-05-15

**问题描述：**
- 用户选择"按句子分段"，输入多个短句后仍只生成 1 段
- 当前文案"按句子分段"让用户误以为"每句独立生成"

**原因：**
- sentence 策略按句子边界识别，但短句仍会按 max_chars 合并到同一 segment

**修复方案：**
- option 文案改为"按句子边界（短句会合并）"
- hint 明确"自动/按句子策略会合并较短内容，直到接近每段上限；如需每条短文本独立生成，请一行一句并选择"每行一段""

**修复位置：**
- `app/static/index.html`：batchStrategy option 和 hint
...
## P12-USAGE-UX5-FIX：HTML 属性引号格式修复

**发现时间：** 2026-05-15

**问题描述：**
- UX5 提交中引入中文弯引号用于 HTML `style` 属性
- 导致浏览器可能解析异常或样式丢失

**修复方案：**
- 将 `style="..."` HTML 属性引号从中文弯引号 `"..."` 改回 ASCII 双引号 `"..."`
- 正文中的中文书名式引号 `"每行一段"` 保持不变

**修复位置：**
- `app/static/index.html`：line 1053

**验证方式：**
- git diff --check：通过 ✅
- Python 字节检查：style 属性使用 ASCII 双引号 (ord=34) ✅
...
## P12-USAGE-FIX6-A0：导入验证试听 audio_asset.duration_ms 缺失
...
**根因分析（FIX6-A0 审查结论）：**
- 链路各节点（ProviderRenderResult、AssetService、AudioAsset、AudioAssetResponse）都正确传递 duration_ms
- `provider_voice_import_service.py` 导入验证使用 `output_format="hex"`
- MiniMax 对 hex 格式可能不返回 `audio_length`，或返回 0
- `estimate_duration_ms()` fallback 使用字符估算（180字/秒），但用户看到的是 0:00
- 最可能根因：MiniMax hex 格式响应中 `extra_info.audio_length` 为 0 或 null

**本次审查输出：**
- `docs/P12_AUDIO_DURATION_PERSISTENCE_AUDIT.md`：完整审查报告

**未改范围：**
- 不改前端 duration 展示（FIX5-B2 已正确）
- 不改 provider API 调用
- 不调用真实 MiniMax

## P12-USAGE-FIX6-B1：AudioAsset.duration_ms fallback 修复

**发现时间：** 2026-05-15

**问题描述：**
- FIX6-A0 审查确认：导入验证试听链路各节点正确传递 duration_ms，但 MiniMax hex 格式返回 0 或 null
- 导致最终 audio_asset.duration_ms 为空

**修复方案：**
- 在 `AssetService.save_assets()` 中增加 duration fallback
- `_valid_duration_ms()` 将任意 value 转为正整数或 None
- `_probe_audio_duration_ms()` 使用 pydub 读取本地音频文件真实时长
- 当 result.duration_ms 无效时，probe 读取本地文件

**修复位置：**
- `app/services/asset_service.py`：新增两个 helper，修改 save_assets() 逻辑
...
## P12-USAGE-UX4-B1：快速绑定面板布局修复

**发现时间：** 2026-05-15

**问题描述：**
- 克隆/设计/导入成功后的快速绑定面板，人设选择框被 `flex:1;min-width:0` 压缩到不可见
- 绑定成功后只有"绑定成功!"，没有"去创作"导向
- import 面板缺少快速试听区

**修复方案：**
- 三处 quick bind 面板改为纵向布局，profile select 固定最小宽度
- 绑定成功后增加"去创作"按钮
- import 面板补齐快速试听区
...
**未改范围：**
- 不改绑定 API、不改克隆/设计/导入主流程

## P12-USAGE-FIX5-B2：高级音色工具音频时长展示

**发现时间：** 2026-05-15

**问题描述：**
- 克隆/设计/导入/试听记录的音频播放位置仍缺少时长展示

**修复方案：**
- voice_import.js：验证音频显示 `duration_ms` 时长，`preload` 改为 `metadata`
...
- `app/static/index.html`（handleGenerateAudition push）
...
## P12-USAGE-FIX5-B1：音频时长展示修复

**发现时间：** 2026-05-15

**问题描述：**
- 核心音频结果区（单条同步/异步/多版本/批量合并）虽有播放功能，但不显式展示总时长
- 后端已提供 `duration_ms` / `total_duration_ms` 字段

**修复方案：**
- 新增 `formatDurationMs()` helper，毫秒转 `x.xs` 格式
- 扩展 `audioPlayerHtml(input)` 支持 `{assetId, durationMs, label, mediaType}`
- 单条/异步/多版本接入 `audio.duration_ms`
- 批量合并音频播放器显示 `total_duration_ms`
- `preload` 从 `none` 改为 `metadata`

**修复位置：**
- `app/static/index.html`：audioPlayerHtml / renderResults / renderAsyncResult / renderBatchResultPlayer
...
**问题描述：**
- 多个音频播放位置虽然可以播放，但不显示总时长
- 后端 `AudioAssetResponse.duration_ms`、`BatchStatusResponse.total_duration_ms` 等字段已支持
- 前端主要依赖浏览器 metadata 显示时长，导致体验不稳定

**审查结论：**

| 严重程度 | 问题 | 根因 |
|---|---|---|
| **P1** | `audioPlayerHtml` 不支持 duration_ms 展示 | 函数设计缺陷 |
| **P1** | 批量结果播放器不显示 `total_duration_ms` | `renderBatchResultPlayer` 未使用该字段 |
| P2 | `audition_records` 不保存 `duration_ms` | push 时遗漏字段 |
| P2 | 克隆/设计/导入 quick bind 面板无时长显示 | 独立拼接，无统一组件 |

**本轮审查输出：**
- `docs/P12_AUDIO_DURATION_AUDIT.md`：完整审查报告

---

## P12-USAGE-CHECK：阶段收口小结

**收口时间：** 2026-05-15

P12 真实使用问题已基本收口。已完成的修复涵盖：
- Workspace / Longtext / Script 音色绑定提示同步
- Advanced tab 提示压缩与 quick bind 布局
- 音频时长展示与后端持久化
- 批量下载入口规范化
- 分段策略文案与 HTML 格式修复
