...
| Tab | 改动 |
|---|---|
| workspace | 顶部 hint card 改为 3 步引导：① 选择人设 → ② 确认音色已绑定 → ③ 输入文本并生成 |
| longtext | 顶部增加提示：长文本批量使用人设绑定的音色；如未绑定，请先去声音库绑定 |
| script | 顶部增加提示：每行台词使用对应人设绑定的音色；未绑定时请先去声音库绑定 |
| voices | 描述 card 更新：声音库用于试听、绑定音色到人设；绑定后可回到工作台或批量任务生成 |
| advanced | 顶部 notice 下增加提示：克隆、设计、导入音色后，建议绑定到人设再用于生成 |
...
仅修改 tab 按钮展示文案：`高级` → `音色工具`

- 不修改 `data-tab="advanced"`
- 不修改 `id="tab-advanced"`
- 不修改任何 JS 逻辑
- Advanced 内部 subtab（声音克隆 / 声音设计 / 导入音色 / 绑定管理 / 删除音色）保持不变
