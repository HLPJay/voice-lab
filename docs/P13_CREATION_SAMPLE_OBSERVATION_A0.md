...
workspace 单条生成（同步 / 异步 / 流式）完成后，返回的 payload 中包含：
- `audio_asset.asset_id`
- `audio_asset.url`
- `audio_asset.duration_ms`
- `text`（用户输入文本）
- `profile_id` / `profile_name`（当前绑定的人设）
- `provider`
- `model`

这些信息在生成结果展示区（resultsArea）已经渲染，但**未抽取为可枚举的样本集合**。

### 3.2 长文本批量结果

batch_longtext 任务提交后，轮询 `/api/voice/batch/status` 获取每个 segment 的状态和音频资产：
- `batch_id`
- `segments[]` — 每个 segment 有 `segment_id`、`text`、`audio_asset.asset_id`、`audio_asset.url`、`audio_asset.duration_ms`、`status`
- 合并音频 `merged_audio.asset_id` / `merged_audio.url` / `merged_audio.duration_ms`

**样本价值：高**，segment 粒度已经切分，每段有独立文本和音频，可以作为样本单元。

**当前局限**：segment 信息在轮询完成后已经在内存中，但未抽取为独立样本记录，也未持久化到 localStorage。

### 3.3 剧本批量结果

batch_script 任务类似 batch_longtext，每行包含：
- `role`（角色名）
- `text`（台词）
- `profile_id`（该行绑定的人设）
- 生成的音频 segment
...
**当前局限**：同 batch_longtext，segment 信息未抽取。
...
`recent_job.js`（`app/static/js/js/recent_job.js`）实际上是 product_hints.js 的一部分，仅负责 voice binding hint 提示逻辑。

真正的"最近任务"存储在 index.html 内联脚本中（`_recentJobs` 数组），写入 `localStorage['recentJobs']`，恢复时通过 `loadRecentJobs()` 读取。

**结论**：`recent_job.js` 不负责样本存储，与样本观察侧边栏无直接关系。后续阶段如果需要借鉴 localStorage 读取逻辑，可以参考 index.html 中的实现，但不应混用语义。

### 3.6 历史记录

`history.js` 从后端 `/api/voice/jobs` 分页获取 VoiceJob 记录，包含：
- `job_id`
- `status`
- `audio_assets[]` — 音频资产数组
- `created_at`
...
**样本价值：中等**，作为后端持久化记录，适合作为样本追溯入口，但：
- 不包含流式生成的中间状态
- 不包含 batch segment 的细粒度信息
- 需要翻页才能看到更多
- 后端 API 依赖

**结论**：history 可以作为样本追溯入口（未来阶段），但不适合作为"创作过程中快速观察"的来源。
