...
P8-2A 到 P8-2D1 已完成。P8-2 已完成音色选择 / 试听工作台的核心产品化。本阶段只做验收、文档对齐和健康检查收口，不新增功能、不改后端、不执行真实 MiniMax smoke test。

## 82. P8-2 全阶段提交链

| 阶段 | 提交 | 说明 |
|---|---|---|
| P8-2A | 601a647 | 音色选择 / 试听工作台现状审查 |
| P8-2B | 5ad8c07 | 音色 tab 信息架构整理 |
| P8-2C | 2483245 | 试听工作台 UI 产品化 |
| P8-2C1 | 4d03901 | 试听工作台收口修复 |
| P8-2D | 8d0a8d0 | 轻量绑定入口整理 |
| P8-2D1 | d763b38 | 移除音色列表行内删除入口 |
...
1. 音色选择 / 试听工作台说明区
2. 音色查询与筛选（provider、类型）
3. 音色搜索
4. `voiceListResults` 动态结果区
5. 试听工作台（选中音色 banner、文本输入、模型选择、生成按钮）
6. 试听记录（全量重绘卡片）
7. 轻量绑定入口（绑定到人设按钮 + quickBindPanel）
8. 绑定说明区

### tab-advanced 当前结构

1. 声音克隆（subtab-clone）
2. 声音设计（subtab-design）
3. 绑定管理（subtab-bindings）
4. 危险操作（subtab-danger）

### 删除音色当前位置

删除音色只保留在：`高级 / 危险操作`。tab-voices 中不再暴露行内删除入口。

## 84. P8-2 当前用户路径验收

### 84.1 查询音色路径

用户进入"音色"tab → 选择 provider → 选择音色类型 → 点击"查询音色" → 使用搜索框过滤 → 查看音色列表

### 84.2 试听路径

点击"试听" → 试听工作台显示当前选中音色 banner → 输入试听文本 → 查看字数提示 → 选择模型 → 点击"生成试听" → 查看试听结果卡片（成功/失败/无音频三种状态）→ 查看试听记录卡片

### 84.3 轻量绑定路径

点击"绑定到人设" → 打开快速绑定面板（标题"绑定到声音人设"）→ 选择声音人设 → 选择模型 → 点击"确认绑定" → 查看绑定成功（绿色卡片）/ 失败（红色卡片）反馈 → 音色列表绑定状态 badge 更新

### 84.4 完整绑定管理路径

进入"高级 / 绑定管理"：查看完整绑定、创建绑定、删除绑定、批量维护绑定

### 84.5 删除音色路径

进入"高级 / 危险操作" → 填写 provider → 填写 voice_id → 选择音色类型 → 点击删除
...
|---|---|
| 音色区功能过多 | ✅ 已通过信息架构整理缓解 |
| 删除音色在普通音色流程中暴露 | ✅ 已移入高级 / 危险操作 |
| 音色列表行内删除入口残留 | ✅ 已在 P8-2D1 移除 |
| 试听结果展示工程化 | ✅ 已在 P8-2C 卡片化 |
| 试听记录局部更新错位 | ✅ 已在 P8-2C1 改为全量重绘 |
| 快速绑定入口文案不清楚 | ✅ 已在 P8-2D 优化 |
| 轻量绑定和高级绑定边界不清 | ✅ 已在 P8-2D 文案说明 |
| 文档与代码状态不一致 | ✅ 已在 P8-2E 收口对齐 |
...
- `renderAuditionWorkstation` ✅
- `updateAuditionSelected` ✅
- `setupAuditionWorkstation` ✅
- `renderAuditionRecords` ✅（全量重绘）
...
- `handleDeleteVoice` ✅
- `handleVoiceDeleteFromList` ✅（函数定义保留，但不再被行内按钮触发）
...
- `parseApiError` ✅
- `switchAdvancedSubtab` ✅

## 88. P8-2 API endpoint 不变验收
...
- ✅ 音色 tab 能正常切换
- ✅ 高级 tab 能正常切换
- ✅ 高级子 tab 能正常切换：声音克隆 / 声音设计 / 绑定管理 / 危险操作
...
- ✅ 试听成功后追加记录卡片
- ✅ 记录卡片包含 voice_id、音色名称、播放器、文本预览、评分、备注、删除按钮
- ✅ 评分星级可交互
- ✅ 备注可输入
- ✅ 删除记录后列表刷新正常
- ✅ 从 0 条到 1 条不会残留"暂无试听记录"
- ✅ 删除中间记录不会出现卡片错位
...
- ✅ 音色列表中不再显示行内"删除"按钮
- ✅ 高级 / 危险操作中存在删除音色表单
- ✅ 删除音色表单包含 provider、voice_id、voice_type、删除按钮
- ✅ 删除音色说明提示高风险

### 管理面板

- ✅ Header 中管理面板入口仍存在
- ✅ `/static/admin.html` 链接未被移除
...
- JS function check：passed
- API marker check：passed
- renderAuditionRecords stability check：passed
- renderVoiceTable inline delete removal check：passed
- Danger delete form retention check：passed
- Advanced subtab mapping check：passed
- documentation marker check：passed
...
- 无 whitespace error
- 无测试失败
- **未执行真实 MiniMax smoke test**（本阶段是验收与文档收口，不涉及后端 API 改造，不需要消耗额度）
...
P8-2 已完成并收口。音色 tab 已从工程管理入口整理为音色选择 / 试听工作台。删除音色能力已收敛到高级 / 危险操作区。下一阶段建议进入 P8-3：任务卡片和结果展示。
...
- 下载入口产品化
- 字幕结果展示优化
- 生成失败 / Resource Guard 提示卡片化
- 不改后端 API

**前端静态文件拆分应作为独立工程化阶段处理，不建议夹在 P8-3 中进行。**
