...
**下一步：** P9-FE1-F2 可正式迁移 `handleBatchScriptSubmit()` 到 `batch_script.js`（Phase 1）。

### 5.3 batch_script.js（建议 P9-FE1-F2 起）

**理由：** 剧本批量状态复杂（`_scriptRows` 数组、角色/台词状态），且与 `populateProfileSelect` 紧耦合，建议分 Phase 迁移。Phase 1 仅迁移 `handleBatchScriptSubmit`，Phase 2 再迁移行管理函数。

**Phase 1 应迁移：**
- `handleBatchScriptSubmit()` 函数整体

**Phase 1 不可迁移（留在 index.html）：**
- 台词行管理函数（`addScriptLine` 等）
- 事件委托逻辑
- 共享轮询/渲染函数
...
| 测试文件 | 覆盖范围 |
|---|---|
| test_frontend_capabilities.py | 主页面加载、capability 控件、provider 切换、失败降级、Admin 页面、Admin 矩阵、剧本 Tab 回归、history.js 模块加载、History Tab 打开与刷新、audition_records.js 模块加载、Voices Tab 打开、audition_records.js 渲染与删除、长文本批量结果 helper 暴露、batch_longtext.js 加载与校验、长文本批量 mock 提交成功启动进度、剧本批量提交校验（共 16 个 E2E） |
