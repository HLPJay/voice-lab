...
**理由：** 历史记录逻辑相对独立，状态简单（`_historyJobs` 数组），与主流程耦合低。

**应迁移：**
- `_historyJobs` 状态
- `loadHistory()`、`renderHistory()`、`renderHistoryJob()`
- 分页和筛选逻辑
- 播放器展开/收起逻辑

### 5.2 audition_records.js
...
| 测试文件 | 覆盖范围 |
|---|---|
| test_frontend_capabilities.py | 主页面加载、capability 控件、provider 切换、失败降级、Admin 页面、Admin 矩阵、剧本 Tab 回归（共 8 个 E2E） |
...
1. **index.html 仍约 5000+ 行**：过大的单文件仍影响可维护性，建议按节奏逐步拆解。
