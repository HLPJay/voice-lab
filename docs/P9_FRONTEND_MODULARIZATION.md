...
- 表单提交与校验
- 单条生成（同步/异步/流式）
- 长文本批量（`handleBatchLongtextSubmit` 待迁入 batch_longtext.js；共享轮询/渲染函数暂留）
- 剧本批量
- 音色试听
- 克隆 / 声音设计 / 导入
- 错误展示与 toast
...
### 5.1 batch_longtext.js（建议下一步）

**理由：** 长文本批量逻辑较剧本批量简单，状态结构相对扁平。

**前提（已完成 P9-FE1-E1）：**
- ✅ `showBatchLongtextResult` / `clearBatchLongtextResult` 已提取为 `window.*` 函数

**应迁移：**
- `handleBatchLongtextSubmit()`
...
| 测试文件 | 覆盖范围 |
|---|---|
| test_frontend_capabilities.py | 主页面加载、capability 控件、provider 切换、失败降级、Admin 页面、Admin 矩阵、剧本 Tab 回归、history.js 模块加载、History Tab 打开与刷新、audition_records.js 模块加载、Voices Tab 打开、audition_records.js 渲染与删除、长文本批量结果 helper 暴露（共 13 个 E2E） |
...
5. **批量共享轮询状态风险**：`_batchPollTimer`、`_currentBatchId`、`_currentBatchPanelId` 为长文本和剧本批量共用，P9-FE1-E 迁移长文本提交函数时需确保共享状态变量不被意外拆分。详见 5.0 节风险分析。
6. **P9-FE1-E1 已解决**：`showBatchLongtextResult` / `clearBatchLongtextResult` 已提取为 `window.*`，嵌套依赖已解除。
