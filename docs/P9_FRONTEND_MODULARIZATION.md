...
| 测试文件 | 覆盖范围 |
|---|---|
| test_frontend_capabilities.py | 主页面加载、capability 控件、provider 切换、失败降级、Admin 页面、Admin 矩阵、剧本 Tab 回归、history.js 模块加载、History Tab 打开与刷新、audition_records.js 模块加载、Voices Tab 打开、audition_records.js 渲染与删除（共 12 个 E2E） |
...
6. **showBatchLongtextResult 嵌套依赖**：`handleBatchLongtextSubmit` 内部闭包引用 DOM id，直接迁移会断环。必须先提取为 `window.showBatchLongtextResult` 才能安全迁移。
