...
```
优先级 1：ContextStore 设计（P14-CONTEXT-B0）
         原因：没有 context 数据，后面的回填和详情都是无根之木

优先级 2：侧边栏详情弹层（P14-PRODUCT-B1）
         原因：用户现在最大困惑是"看不到完整内容"

优先级 3：长文本回填（P14-CONTEXT-B1）
         原因：长文本是主生产入口，回填价值高

优先级 4：剧本回填（P14-CONTEXT-B2）
         原因：剧本结构复杂，回填设计难度高于长文本

优先级 5：全局侧边栏可见性（P14-PRODUCT-B0）
...
| 阶段 | 内容 | 前提 |
|------|------|------|
| P14-CONTEXT-B0 | ContextStore 数据结构设计 | P14-A0 完成 |
| P14-PRODUCT-B0 | 全局 SampleSidebar 可见性方案设计 | P14-A0 完成 |
| P14-PRODUCT-B1 | 侧边栏卡片预览与详情弹层设计 | P14-A0 完成 |
...
```
如果目标是提升真实生产效率，长文本 / 剧本回填优先级高于 Workspace UI polish。
建议优先做 P14-CONTEXT-B0 + P14-CONTEXT-B1。
```
